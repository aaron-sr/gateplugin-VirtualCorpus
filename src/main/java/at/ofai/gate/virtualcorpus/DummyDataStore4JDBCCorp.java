...
/**
 * A dummy datastore so that documents that are returned by a JDBCCorpus are not
 * looking like transient documents. This will help getting them unloaded.
 * 
 * @author Johann Petrak
 */
public class DummyDataStore4JDBCCorp extends DummyDataStore4Virtuals {
	 ...
	 * <p>
	 * Advantage when unloading: manual changes are always saved. Disadvantae no
	 * control if we want to save or not.
	 *
	 * @param lr
	 ...
	 * corpus allows to directly add a document to the corpus. All documents that
	 * are added to the corpus will get their datastore set to this data store. If
	 * they already have a different data store set, an exception is thrown. If
	 * adopt is called via e.g. doc.getDataStore.adopt(doc) then it is noop, because
	 * a doc where getDataStore returns this datastore has already been added to the
	 * directory corpus. If adopt is called via doc.getDataStore.adopt(someotherlr)
	 * an exception is thrown since the datastore cannot adopt any other LRs.
	 * 
	 * @param lr
	 * @param secInfo
	 * @return
		...
		if (lr instanceof Document) {
			...
			if (doc.getDataStore() == null || doc.getDataStore() != this) {
				throw new PersistenceException(
						"Cannot adopt document, already in a different datastore: " + lr.getName());
			}
			...
			if (corpus.getDataStore() != null) {
				throw new PersistenceException("Cannot adopt corpus " + corpus.getName()
						+ " which belongs to datastore " + corpus.getDataStore().getName());
			}
			if (corpus.size() != 0) {
				throw new PersistenceException("Cannot adopt corpus " + corpus.getName()
						+ " which is non empty, number of documents contained: " + corpus.size());
			}
			...
			try {
				Resource newCorpus = Factory.createResource("at.ofai.gate.virtualcorpus.JDBCSubsetCorpus", parms,
	...
	@Override
	public void close() {
		System.err.println("This resource cannot be closed, it will be closed automatically when JDBCCorpus "
				+ getCorpusName() + " is closed");
	}
