 ...
 * JDBCCorpus (its "parent corpus"). Only documents that are in the parent
 * corpus can be added, and removing a document from the SubsetCorpus will not
 * delete any files in the JDBC table. This corpus is essentially just a view
 * into the parent corpus. Its main purpose is to support the Learning plugin
 * with a JDBCCorpus.
 * <p>
 * NOTE: for now, only a non-transient JDBCCorpus can have a JDBCSubsetCorpus.
 * <p>
 ...
 */
@CreoleResource(name = "JDBCSubsetCorpus", interfaceName = "gate.Corpus", icon = "corpus", helpURL = "http://code.google.com/p/gateplugin-virtualcorpus/wiki/JDBCCorpusUsage", comment = "A corpus that provides a view of a subset of the documents in an existing JDBCCorpus")
public class JDBCSubsetCorpus extends VirtualSubsetCorpus implements Corpus, CreoleListener {
	...
	/**
	 */
	@CreoleParameter(comment = "The JDBCCorpus for which to create this corpus", defaultValue = "")
