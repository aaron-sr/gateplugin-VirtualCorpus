...
/**
 * A corpus that contains a subset of the documents of an existing VirtualCorpus
 * (its "parent corpus").
 * 
 * Only documents that are in the parent corpus can be added, and removing a
 * document from the SubsetCorpus will not remove any documents from the parent
 * corpus, nor will it remove the original file or database table row which
 * backs the document. A Subset Corpus is essentially just a view into the
 * parent corpus. Its main purpose is to support the Learning plugin with a
 * VirtualCorpus and other situations where one wants to create corpora from
 * documents in an existing corpus for subsetting, sampling etc.
 * <p>
 ...
 * datastore of an existing JDBCCorpus. However, it can also be instantiated
 * directly (the only required parameter is an existing JDBCCorpus).
 * 
 * @author Johann Petrak
 */
public abstract class VirtualSubsetCorpus extends AbstractLanguageResource implements Corpus, CreoleListener {
	...
	// protected Map<String,Document> loadedDocuments = new
	...
	/**
	 * Test if the document with the given index is loaded. If an index is specified
	 * that is not in the corpus, a GateRuntimeException is thrown.
	 * <p>
	 * This is handled entirely by the parent JDBCCorpus but is valid only if
	 * requested for a document that had been added to this corpus.
	 * 
	 * @param index
	...
	/**
	 * This follows the convention for transient corpus objects and always returns
	 * false.
	 * 
	 * @return always false
	 */
	 ...
	 * only the name is checked! Otherwise check if the document is in the parent
	 * corpus, if yes, add it to this corpus by adding the name and the original
	 * index.
	 */
	public boolean add(Document doc) {
		...
		if (index != null) {
			...
			if (parentIndex == null) {
				throw new GateRuntimeException("Attempt to add a document to JDBCSubsetCorpus " + this.getName()
						+ " but no document with that name is in the parent jdbc corpus " + virtualCorpus.getName());
			}
	 ...
	 * the saveDocuments parameter is set to false.
	 */
	public void clear() {
	...
	/**
	 * Remove a document from this corpus. This will just remove it from the view
	 * but always leave the files and documents in the parent corpus untouched!
	 * <p>
	...
	public Document remove(int index) {
		if (index < 0 || index >= documentNames.size()) {
			throw new GateRuntimeException("Attempt to remove document with index " + index + "from JDBCSubsetCorpus "
					+ this.getName() + " of size " + documentNames.size());
		}
		String docName = documentNames.get(index);
		documentNames.remove(index);
		originalIndexes.remove(index);
		documentIndexes.remove(docName);
		fireDocumentRemoved(new CorpusEvent(this, null, // can we get away with using null instead of doc?
	...
	public boolean remove(Object docObj) {
		...
		originalIndexes.remove(index);
		documentIndexes.remove(docName);
		fireDocumentRemoved(new CorpusEvent(this, null, index, CorpusEvent.DOCUMENT_REMOVED));
		return true;
	}
	...
	protected String notImplementedMessage(String methodName) {
		return "Method " + methodName + " not implemented for corpus " + this.getName() + " of class "
				+ this.getClass();
	}
